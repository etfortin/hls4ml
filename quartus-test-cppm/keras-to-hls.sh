...
mkdir -p "${basedir}" #cria diretorio chamado quartus_proj

for model in "${models[@]}"  #${models[@]} é o Keras_LSTM
do
   name=${model}
   h5=${name}"_weights"
   ...
   file="${basedir}/${base}-${pycmd}.yml"  #Documentos são salvos em formato de texto simples e são adicionados com a .yml
